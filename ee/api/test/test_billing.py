...
class TestUnlicensedBillingAPI(APIBaseTest):
    @parameterized.expand(
        [
            ("get", "/api/billing/", None, {"products": [], "current_total_amount_usd": "0"}),
            ("get", "/api/billing/get_invoices/", None, {"count": 0, "link": "/organization/billing"}),
            ("get", "/api/billing/credits/overview/", None, {"status": "none", "credit_brackets": []}),
            ("get", "/api/billing/coupons/overview/", None, {"claimed_coupons": []}),
            ("post", "/api/billing/activate/", {}, {"success": True}),
            ("post", "/api/billing/activate/authorize/status/", {}, {"status": "success"}),
            ("patch", "/api/billing/license/", {"license": "example"}, {"success": True}),
        ]
    )
    def test_local_billing_without_license(self, method: str, url: str, data: dict | None, expected: dict) -> None:
...
class TestBillingAPI(APILicensedTest):
    def test_local_billing_accepts_legacy_license(self) -> None:
        self.license.key = "test_key"
        self.license.save()
        TEST_clear_instance_license_cache()
        response = self.client.get("/api/billing/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["available_product_features"], self.organization.available_product_features)
...
class TestPortalBillingAPI(APILicensedTest):
    ...
    def test_portal_returns_local_billing_page(self) -> None:
        response = self.client.get("/api/billing/portal/")
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["Location"], "/organization/billing")
...
class TestBillingPermissionDeniedForMembers(APILicensedTest):
    ...
    def test_list_still_accessible(self) -> None:
        response = self.client.get("/api/billing/")
        self.assertEqual(response.status_code, 200)
