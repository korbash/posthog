...
Общий `csp_report_endpoint()` возвращает пустой адрес при запрещённой отправке.
Это сохраняет upstream-логику CSP, включая nonce и `form-action`, но middleware
больше не добавляет ни в одном режиме:
...
ограничение истории. После переноса upstream тест обращается к
`posthog.models.activity_logging.retention`. Общий модуль используется и API,
и SQL-проверками истории; отдельный старый `api/advanced_activity_logs/utils.py`
не восстанавливается. Неподдерживаемая единица при заданном числовом лимите
по-прежнему обрабатывается upstream fallback.
...
## Обновление upstream от 7 сентября 2026 года

- Предыдущая ветка: `050f60ca7b141cbc89a37461511b3d1c1fe6e285`.
- Предыдущая база: `f292f19440b` от 28 августа.
- Входящий `master`: `07c5f4c8468c97ab091c22e68e260adeb2c47b64`.
- Объём upstream: 1623 коммита, 9217 изменённых файлов.
- Семь конфликтующих файлов: оба layout, `apps.py`, middleware и его тесты,
  тесты proxy records, удалённый upstream модуль ограничений audit logs.

Конфликты разрешены с сохранением новых security-настроек CSP и обработчиков
кэша организаций. Запрет CSP-отчётов перенесён в общий helper, отсутствие
ограничения истории audit logs перенесено в новый общий модуль. Новые metrics
вызовы Signals используют отключённый SDK; существующий тест клиента дополнен
вызовом metrics и flush с проверкой отсутствия HTTP-запросов.

На объединённой версии прошли 113 backend-тестов: SDK и metrics, режимы запуска,
usage reports, billing, CSP и оба набора проверок audit-log retention.
Дополнительные 160 backend-тестов организаций, proxy records, template context,
license usage и отсутствия записей billing-квот также прошли.
Прошли также три frontend-теста загрузчика SDK и adblock detection и один
Node.js тест отсутствия billing-блокировки ingestion.
Полная генерация OpenAPI, API-клиентов, MCP-инструментов и scopes завершилась
без изменений в сгенерированных файлах. Первый запуск внутри preflight достиг
его лимита 600 секунд; отдельный `hogli build:openapi` завершился успешно.
Полный frontend typecheck сначала обнаружил несобранные Quill и HogVM.
Оба пакета собраны штатными командами; повторный typecheck остановлен из-за
нехватки оперативной памяти и активного swap. Успех этой проверки не подтверждён.
Общий `uv run mypy --cache-fine-grained .` завершился без ошибок в 19 876 файлах.
Preflight проверил Python lint/format, Markdown, lock-файлы, workflows и конфликты
миграций. Результаты в следующем разделе относятся к предыдущей базе.

Проверка работающего web, workers и браузера с блокировкой Cloud и записью
попыток соединения не проводилась. Запущена только локальная инфраструктура для
backend-тестов; полноценный ingestion-сценарий с появлением события в интерфейсе
не проверен. Успешные целевые тесты не заменяют эту проверку автономного режима.

При просмотре шаблонов обнаружен существовавший до merge внешний iframe в
`posthog/templates/500.html`: страница ошибки загружает
`https://posthog.com/service-error`. Upstream изменил только его форматирование.
Этот путь не заблокирован текущим патчем и требует отдельного исправления, если
необходимо исключить любые автоматические обращения браузера к PostHog.

## Проверки до обновления upstream
