...
/**
 * Detects whether configured PostHog requests are being blocked (e.g. by an adblocker).
 *
 * When an analytics host is available, two checks are performed:
 ...
 *     blocked.
 *
 * Without an analytics host, the check returns `ok` without a network request.
 * The result is only surfaced after `delayMs` (default 20 s) so we don't flash
 * a warning before the user has had time to wait.
 */
export function useAdblockDetection(delayMs: number = 20_000): AdblockDetectionResult {
    ...
    useEffect(() => {
        ...
        const detect = async (): Promise<AdblockDetectionResult> => {
            ...
            try {
                await fetch(`${window.JS_POSTHOG_HOST}/decide/?v=3`, {
                    method: 'POST',
                    mode: 'no-cors',
                    body: JSON.stringify({}),
                })
